...
class ConvolutionDepthwiseTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
      ...
      auto weightInfo = weight->getInfo();
      auto weightTensorData = weight->readMap<float>();
      if (op->name()->c_str() == std::string(
          "BiseNetV2/semantic_branch/stage_3/ge_block_1_repeat_1/gather_expansion_block/stride_equal_two_module/depthwise_conv_block_1/depthwise_conv_output")) {
          int lms = 10;
      }
      if (!weightInfo || !weightTensorData) {

...
Pipeline::Pipeline(std::vector<Schedule::PipelineInfo> &&infos,
                   std::shared_ptr<Backend> backend,
                   std::shared_ptr<Backend> cpuBackend,
                   std::shared_ptr<Backend> constBackend,
                   bool allocInput,
                   bool outputStatic,
                   const TuningAttr &tune,
                   const Runtime *rt,
                   const Runtime *cpuRt,
                   CacheExecutionMap &cacheMap) : mOriginExecution(cacheMap)
#ifndef MNN_BUILD_MINI
...
ErrorCode Pipeline::encode(bool isStatic, bool supportDebug) {
    ...
    if (isStatic) {
        ...
        auto res =
            GeometryComputerUtils::shapeComputeAndGeometryTransform(mInfo, mContext, mBackupBackend, mUseGeometry);
        if (res != NO_ERROR) {
            return res;
        }
#endif
...
void Pipeline::_pushTuningTask(std::vector<Schedule::PipelineInfo> &&initInfos) {
    ...
    auto future = std::async(std::launch::async,
                             [&, this](std::vector<Schedule::PipelineInfo> &&infos,
                               ...
                               for (auto &info : infos) {
                                   ...
                                   for (auto &iterP : buffer.command) {
                                       ...
                                       const_cast<Runtime *>(mRuntime)->onMaskOpReady(iter.inputs,
                                                                                      iter.outputs,
                                                                                      iter.op);

                                       // If create op failed, we can also mask the op is ready for runtime
                                       std::shared_ptr<Execution>
                                           exe(backend->onCreate(iter.inputs, iter.outputs, iter.op));
                                       if (nullptr == exe) {
                                           continue;
                                       }
...
void _reorderOpCommandList(Tensor *reuseTensor,
                           Tensor *replacedTensor,
                           std::vector<SharedPtr<Command>>::iterator &cmdIterP,
                           std::vector<Schedule::PipelineInfo> &info) {
...
ErrorCode Pipeline::allocMemory(bool firstMalloc) {
    ...
    for (auto &info : mInfo) {
        ...
        for (auto iterP = buffer.command.begin(); iterP != buffer.command.end(); ++iterP) {
            ...
            if (iter.op->name() != nullptr) {
                std::string lmsName = iter.op->name()->c_str();
                std::cout << lmsName << std::endl;
                if (lmsName == "din/din_layer/Tile") {
                    int a = 10;
                }
            }
